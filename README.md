...
| Menu: navegar/marcar mecânicas (músicas suas) | A/D movem o cursor no painel de checkboxes · **C** liga/desliga a linha em foco |
...
Com a música selecionada no menu, um **painel de checkboxes** deixa montar sua própria combinação de mecânicas — não mais um punhado de combinações fixas pré-prontas:

- **A/D (ou ←/→)** movem o cursor entre as linhas do painel — a primeira é sempre `< DEFENSOR >`/`< ARCADE 4K >` (o modo base), as demais são os modifiers daquele modo.
- **C** liga/desliga a linha em foco — na linha de modo, alterna Defensor↔Arcade 4K (e a lista de linhas abaixo muda de acordo); numa linha de modifier, marca/desmarca o checkbox.
- **ENTER, ESPAÇO ou CLIQUE** jogam com a combinação atual; **T** liga/desliga o Modo Treino, como sempre.

O Defensor mostra `telegraph_rings`, `polarity`, `orbital_shields`, `twin_threats`, `orbital_eclipses`, `overload` e `holds`; o Arcade 4K só `holds` (compartilhado pelos dois modos). `"radius_collapse"`/`"bombs"`/`"heal"` **não** entram no painel: só fazem algo visível em cima de dado específico de fase curada (eventos `modchart_events`/ameaças desses tipos no beatmap) que uma música do jogador nunca tem — um checkbox que não muda nada na tela é pior que não oferecer a opção. Marcar `"holds"` desmarca `"polarity"` automaticamente e vice-versa (e são mutuamente exclusivos — reusam o mesmo `threat_type` "pesada" com significados incompatíveis). Toda música começa sem nenhum modifier marcado; a combinação escolhida vira `HertzConfig.active_modifiers` direto na hora de confirmar (ver [Modificadores](#modificadores)).
...
**Nos menus.** O seletor de minigame das músicas do jogador passou por 2 desenhos: primeiro um ciclo de 10 combinações FIXAS (`MODE_CYCLE`), depois um **painel de checkboxes** que deixa marcar/desmarcar cada modifier individualmente (ver [Use a sua própria música](#use-a-sua-própria-música-arraste-e-jogue)) — a demonstração mais literal da tese de "combinar mecânicas livremente". `HertzGameLoop` guarda, por fase `selectable_mode`, um `game_mode` ("defender"/"lanes"), um `frozenset` de modifiers marcados e um cursor de linha; `modifier_rows_for_game_mode(game_mode)` decide quais linhas aparecem (a primeira é sempre a linha especial de troca de modo, `GAME_MODE_ROW`). A/D movem o cursor, uma nova tecla **C** (`toggle_modifier`, adicionada só pra isso — `pygame.key.key_code(...)` resolve qualquer nome de tecla pygame genericamente, zero mudança na engine) liga/desliga a linha em foco. Marcar `"holds"` desmarca `"polarity"` automaticamente e vice-versa (mutuamente exclusivos, mesmo `threat_type` "pesada").

O menu de fases curadas (`stages.json`) já resolvia a dica de controles a partir de `stage.active_modifiers` desde a introdução da arquitetura; essa resolução tem uma cadeia de prioridade (`overload` > `orbital_shields` > `twin_threats` > `orbital_eclipses` > `polarity` > `holds` > genérico do modo) para que uma fase com vários modifiers ligados ao mesmo tempo (ex.: "5 - Pesadelo") mostre a dica mais relevante, nunca uma lista de 7 linhas. **Achado real ao construir isso**: `HBPygameRenderer._blit_centered`/`_draw_modifier_row` só centralizam uma `Surface` já pronta — nunca quebram linha nem escalam — então um texto de dica, um rótulo de fase (`stage.name` + `stage.subtitle`) ou um rótulo de checkbox comprido demais estoura as bordas da janela em vez de aparecer cortado educadamente; 3 rótulos/dicas estouravam (2 novos, 1 pré-existente em "8 - Arcade: Notas Longas") até serem encurtados, e 2 testes de regressão (`test_render_visuals.py`, medem a largura renderizada de verdade com `pygame.font.Font(...).size(...)` contra `config.window_width`) passaram a cobrir isso.
