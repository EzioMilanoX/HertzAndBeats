...
- **Paletas Cosméticas** — `hertzbeats/palettes.py` (`PALETTE_CATALOG`): recolore os 2 tints de Polaridade (`HertzConfig.threat_blue_rgb`/`threat_pink_rgb`, threadados até `RadialRhythmSpawnerSystem`) — o Anel de Convergência já herda a cor da ameaça que o gerou, então nenhuma mudança adicional foi necessária ali. 3 paletas: **Clássica** (Azul/Rosa, sempre disponível), **Dourado/Prata** (exige Rank S nalguma fase/música salva) e **Neon Verde/Roxo** (exige Rank SS) — `unlocked_palette_ids`, função pura sobre `player_progress.json`. Escolhida no Vault (A/D cicla só entre as desbloqueadas), persistida em `user_settings.json` (`palette_id`, campo independente da latência calibrada — os dois nunca se apagam um ao outro) e aplicada à PRÓXIMA fase carregada (nunca à fase em andamento).
