...
Com a música selecionada no menu, **A/D (ou ←/→) escolhem o minigame** entre 10 variantes (`MODE_CYCLE` em `hertz_game_loop.py`) — e ENTER joga:

| Variante | `active_modifiers` |
| --- | --- |
| O Defensor | `()` |
| Arcade 4K | `()` |
| Defensor: Polaridade | `("telegraph_rings", "polarity")` |
| Defensor: Notas Longas | `("telegraph_rings", "holds")` |
| Arcade 4K: Notas Longas | `("holds",)` |
| Defensor: Escudos Rotativos | `("telegraph_rings", "polarity", "orbital_shields")` |
| Defensor: Gêmeos de Polaridade | `("telegraph_rings", "polarity", "twin_threats")` |
| Defensor: Eclipses Orbitais | `("telegraph_rings", "polarity", "orbital_eclipses")` |
| Defensor: Overload do Núcleo | `("telegraph_rings", "polarity", "overload")` |
| Defensor: Pesadelo | os 4 modifiers acima de uma vez |

Cada uma é o modo base com uma combinação fixa de `active_modifiers` ligada via `MODE_VARIANT_OVERRIDES` (ver [Modificadores](#modificadores)) — as mesmas mecânicas das fases curadas, agora disponíveis para qualquer música sua, todas sobre o **mesmo beatmap**. As 5 últimas expõem o 3º pacote hardcore do Defensor (sempre com `"polarity"` junto — todas dependem dela); `"radius_collapse"`/`"bombs"`/`"heal"` **não** entraram no ciclo porque só fazem algo visível em cima de dado específico de fase curada (eventos `modchart_events`/ameaças desses tipos no beatmap) que uma música do jogador nunca tem — ligar só o modifier seria um item de menu que não muda nada na tela.
...
**Nos menus.** O seletor de minigame das músicas do jogador (`MODE_CYCLE` em `hertz_game_loop.py`, ver [Use a sua própria música](#use-a-sua-própria-música-arraste-e-jogue)) foi estendido de 5 para 10 variantes, expondo o 3º pacote hardcore do Defensor um modifier de cada vez. O menu de fases curadas (`stages.json`) já resolvia a dica de controles a partir de `stage.active_modifiers` desde a introdução da arquitetura; essa resolução ganhou uma cadeia de prioridade (`overload` > `orbital_shields` > `twin_threats` > `orbital_eclipses` > `polarity` > `holds` > genérico do modo) para que uma fase com vários modifiers ligados ao mesmo tempo (ex.: "5 - Pesadelo") mostre a dica mais relevante, nunca uma lista de 7 linhas. **Achado real ao adicionar isso**: `HBPygameRenderer._blit_centered` só centraliza uma `Surface` já pronta — nunca quebra linha nem escala — então um texto de dica ou um rótulo de fase (`stage.name` + `stage.subtitle`) comprido demais estoura as bordas da janela em vez de aparecer cortado educadamente; 3 rótulos/dicas estouravam (2 novos, 1 pré-existente em "8 - Arcade: Notas Longas") até serem encurtados e um teste de regressão (`test_render_visuals.py`, mede a largura renderizada de verdade com `pygame.font.Font(...).size(...)` contra `config.window_width`) passar a cobrir isso.
