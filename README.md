...
- **Multiplicador de Pontuação** — prévia AO VIVO no Pré-Voo (`compute_score_multiplier`, pura, soma `MODIFIER_SCORE_BONUS` de cada modifier ligado — Eclipses/Gêmeos `+20%`, Escudos/Overload/Colapso de Visão/Bombas `+15%`, Polaridade/Holds `+10%`, Anéis `0%`, Cura `−5%` — e aplica `−50%` se o Modo Treino estiver ligado, com piso em `MIN_SCORE_MULTIPLIER`). Calculada UMA vez na composição (nunca por acerto em tempo real) e aplicada ao `score_perfect`/`score_good` efetivos — incentiva ligar os modifiers cruéis para tentar um Rank SS com pontuação astronômica.
- **Arquivos (Vault)** — agregados globais e só-leitura de `player_progress.json`: fases vencidas de quantas existem, medalhas totais, e quantas vezes cada Rank já foi o melhor alcançado nalguma fase/música.
